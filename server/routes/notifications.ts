...
export async function notificationsIndex(req: VercelRequest, res: VercelResponse) {
    ...
    if (req.method === 'GET') return notificationsGet(req, res, user.id);
    if (req.method === 'PATCH') return notificationsPatch(req, res, user.id);
    return res.status(405).json({ error: 'Method not allowed' });
}

async function notificationsGet(req: VercelRequest, res: VercelResponse, userId: string) {
    try {
...
async function notificationsPatch(req: VercelRequest, res: VercelResponse, userId: string) {
    ...
    try {
        ...
        const { error } = await supabase.from('notifications').update({ is_read: true }).eq('id', notification_id).eq('user_id', userId);
        if (error) return res.status(400).json({ error: error.message });
