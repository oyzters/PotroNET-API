...
// GET|POST /resources
export async function resourcesIndex(req: VercelRequest, res: VercelResponse) {
    if (req.method === 'GET') return resourcesGet(req, res);
    if (req.method === 'POST') return resourcesPost(req, res);
    return res.status(405).json({ error: 'Method not allowed' });
}

async function resourcesGet(req: VercelRequest, res: VercelResponse) {
    try {
        ...
        let query = supabase.from('resources')
            .select(`*, uploader:profiles!resources_user_id_fkey(id, full_name, avatar_url, email), career:careers(id, name)`, { count: 'exact' })
...
async function resourcesPost(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        const { data, error } = await supabase.from('resources')
            .insert({
                user_id: user.id, title, description: description || '', resource_type,
                file_url: file_url || '', career_id: career_id || null,
                subject_name: subject_name || '', professor_name: professor_name || '',
            })
            .select(`*, uploader:profiles!resources_user_id_fkey(id, full_name, avatar_url, email), career:careers(id, name)`)
            .single();
        if (error) return res.status(400).json({ error: error.message });
